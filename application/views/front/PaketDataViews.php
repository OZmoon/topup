              ...
              for (var i = 0; i < data.data.length; i++) {
                ...
                if (data.data[i].code != "BALANCE") {
                  ...
                  $(".listpayment").append('<div class="row p-1 mt-2 rounded mx-1" style="height:50px;">'
                    ...
                    +'<?php if ($this->ion_auth->logged_in()){ ?>'
                      +'<div class="col-md-8 col-8">'
                        +'<p style="width:100%;"><a style="width:100%;" id="'+data.data[i].code+'"  class="readmore text-center text-dark payment rounded '+data.data[i].code+'">'+formatRupiah(data.data[i].price.toString(), 'Rp. ')+'</a></p>'
                      +'</div>'
                    +'<?php }else{ ?>'
                      +'<div class="col-md-8 col-8">'
            ...
            success: function (data) {
              if (data === null) {
                ...
                $.each(data.dataCat,function(key,value){
                  $(".listpulsa").append('<div class="col-md-12 col-12 rounded text-dark" style="margin-top:10px;margin-bottom:10px;padding:5px;background:#e8f953">'
                              +'<h5 class="m-0">'+key+'</h5>'
                              +'</div>');
                    $.each(data.data,function(key2,value2){
                      if (key == value2.type && value2.brand == ops['operator']) {
                        $(".listpulsa").append('<div class="col-md-4 col-6">'
                          +'<p style="width:100%;"><button price="'+value2.newprice+'" style="width:100%;" id="'+value2.buyer_sku_code+'"  class="readmore text-center text-dark item rounded ">'+value2.product_name+'</button></p>'
                          +'</div>');
                      }
                    });
                });
