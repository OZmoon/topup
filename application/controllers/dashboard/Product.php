...
class Product extends CI_Controller
{
    ...
    public function api()
    {
        ...
        curl_setopt_array($curl, array(
          CURLOPT_URL => "https://kaneki-pedia.site/api/getproduct",
    ...
    public function submit()
    {
        ...
        $ProductApi = $this->input->post('ProductApi');
        $ProductCek = $this->input->post('ProductCek');
        $ProductType = $this->input->post('ProductType');
        $ProductCat = $this->input->post('ProductCat');
        $Status = $this->input->post('Status');
        $ProductSlug = url_title(strtolower($ProductName));
    ...
    public function allaktif()
    {
      ...
      // exit();
      foreach ($dataDb as $d) {
        $this->db->where('Id',$d->Id)->update('data_product',$dataUpdate);
      }
      $this->session->set_flashdata('message', 'Data berhasil diubah.');
      redirect('dashboard/product/');
    }
    public function allnonaktif()
    {
      $dataUpdate = [
        'ProductStatus' => 0
      ];
