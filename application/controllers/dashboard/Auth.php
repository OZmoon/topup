...
class Auth extends CI_Controller
{
	...
	public function index()
	{
		...
		else if (!$this->ion_auth->is_admin()) // remove this elseif if you want to enable this for non-admins
		{
			...
			redirect('dashboard/home', 'refresh',$this->data);
		}
		else
		{
			$this->data['title'] = $this->lang->line('index_heading');
	...
	public function mv(){
		$this->session->set_flashdata('message', 'CEK AJA');
	...
	public function addsaldo($id){
		...
		$dataDb = $this->db->where('id',$id)->get('users')->row();
		$data = [
			'title' => SITE_NAME.' | Add Saldo',
			'page' => 'edit',
			'data' => $dataDb
		];
		$this->load->view('auth/AddSaldoView',$data);
	}
	...
	$AddBalance = $this->input->post('Balance');
	$Balance = $this->input->post('BalanceOld');
	...
	if ($this->db->where('id',$id)->update('users',$updateData)) {
		...
		$refid = REF.date("md").mt_rand(1,100).date("hsi").mt_rand(1000,9999).$dataRow;
		$inpuData = [
			'UserId' => $id,
			'PaymentMetod' => 'ADMIN',
			'InvoiceId' => $refid,
			'UniqAmount' => $AddBalance,
			'Balance' => $AddBalance,
			'Tanggal' => date('Y-m-d'),
			'Status' => 1,
		];
		$insert = $this->db->insert('data_deposit', $inpuData);
		$this->session->set_flashdata('message', 'Saldo berhasil ditambahkan.');
		redirect('dashboard/auth');
	}else {
	...
	public function change_password()
	{
		...
		else
		{
			...
			{
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				redirect('dashboardauth/change_password', 'refresh');
			}
		}
	}
	...
	public function register()
	{
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			$additional_data = [
				...
				'Balance' => $WebsiteSetting->BonusRegist,
			];
		}
		...
		else
		{
			...
			$this->session->flashdata('message',$this->ion_auth->messages());
	...
	public function create_user()
	{
		...
		else
		{
			...
			$this->session->flashdata('message','CECE');
	...
	*/
	...
	public function edit_user($id)
	{
		...
		$this->form_validation->set_rules('balance', 'Balance perlu diisi.', 'trim|required');
		...
		$this->data['balance'] = [
			...
			'id'    => 'balance',
			'type'  => 'number',
			'value' => $this->form_validation->set_value('balance', $user->Balance),
			'class' => 'form-control',
		];
		$this->data['company'] = [
			'name'  => 'company',
